...
OccFilterNArea_COUNTY <- function(crop_dir) {
  ...
  naDir2 = paste(crop_dir, "/biomod_modeling/native-areas/polyshps", sep="")
  NAREA_COUNTY_DIR<-paste0(crop_dir, "/NAREAS/SHP", sep="")
  ...
  for(sp in spList){
    ...
    spName<-sp;spName<-sub(".csv","",spName)
    ...
    if(file.exists(paste(NAREA_COUNTY_DIR, "/", spName, "/narea.shp", sep=""))){
      
      naDir<-NAREA_COUNTY_DIR
    }else{
      naDir<-naDir2
    ...
    if(!file.exists(narea)){
      ...
      names(occ) = c("lon","lat")
      write.csv(occ, paste(outDir, "/", sp,"_NAREA",".csv", sep=""), row.names=FALSE)
    }else{
