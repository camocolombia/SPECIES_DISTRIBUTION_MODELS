...
ENM_PA_MEDIAN <-read.csv(paste0(summ_dir,"/","NAREAS","/","ensemble_metrics_PA_MEDIAN.csv"),header = T)
ENM_PA_MEDIAN<-ENM_PA_MEDIAN[,1:11]
ENM_PA_SUM    <-read.csv(paste0(summ_dir,"/","NAREAS","/","ensemble_metrics_PA_SUM.csv"),header = T)
ENM_PA_SUM<-ENM_PA_SUM[,1:11]

ENM_PA_MEAN   <-read.csv(paste0(summ_dir,"/","NAREAS","/","ensemble_metrics_PA_MEAN.csv"),header = T)
...
########################################################################################
 nullModel <- read.csv(paste(crop_dir, "/maxent_modeling/summary-files/nullModel_auc_NAREA.csv", sep=""))
 ...
 newpriFile<-ENM_PA_MEAN
  ...
  #ENM_PA_MEAN<-ENM_PA_MEAN[nullModel$SPID,]
  ENM_PA_MEAN$nAUC <- merge(ENM_PA_MEAN,nullModel,by.x="Taxon",by.y="SPID")$AUC
  ENM_PA_MEAN$nAUC[which(is.na(ENM_PA_MEAN$nAUC))]<-0.5
  ENM_PA_MEAN$auc[which(is.na(ENM_PA_MEAN$auc))]<-0.5
  ENM_PA_MEAN$cAUC[which(is.na(ENM_PA_MEAN$cAUC))]<-0
  
  
 newpriFile<-ENM_PA_MEDIAN
 ENM_PA_MEDIAN$cAUC <- calc_cauc(as.character(newpriFile$Taxon))
 # ENM_PA_MEDIAN<-ENM_PA_MEDIAN[nullModel$SPID,]
 ENM_PA_MEDIAN$nAUC <- merge(ENM_PA_MEDIAN,nullModel,by.x="Taxon",by.y="SPID")$AUC
 ENM_PA_MEDIAN$nAUC[which(is.na(ENM_PA_MEDIAN$nAUC))]<-0.5
 ENM_PA_MEDIAN$auc[which(is.na(ENM_PA_MEDIAN$auc))]<-0.5
 ENM_PA_MEDIAN$cAUC[which(is.na(ENM_PA_MEDIAN$cAUC))]<-0
 
 
 newpriFile<-ENM_PA_SUM
 ENM_PA_SUM$cAUC <- calc_cauc(as.character(newpriFile$Taxon))
 # ENM_PA_SUM<-ENM_PA_SUM[nullModel$SPID,]
 ENM_PA_SUM$nAUC <- merge(ENM_PA_SUM,nullModel,by.x="Taxon",by.y="SPID")$AUC
 ENM_PA_SUM$nAUC[which(is.na(ENM_PA_SUM$nAUC))]<-0.5
 ENM_PA_SUM$auc[which(is.na(ENM_PA_SUM$auc))]<-0.5
 ENM_PA_SUM$cAUC[which(is.na(ENM_PA_SUM$cAUC))]<-0
...
write.csv(ENM_PA_SUM, paste0(summ_dir,"/","NAREAS","/","ensemble_metrics_PA_SUM.csv"),row.names=F,quote = F)
write.csv(ENM_PA_MEAN, paste0(summ_dir,"/","NAREAS","/","ensemble_metrics_PA_MEAN.csv"),row.names=F,quote = F)
write.csv(ENM_PA_MEDIAN, paste0(summ_dir,"/","NAREAS","/","ensemble_metrics_PA_MEDIAN.csv"),row.names=F,quote = F)
write.csv(VALID_MODELS, paste0(summ_dir,"/","NAREAS","/","VALID_MODELS.csv"),row.names=F,quote = F)
write.csv(ModelsMets, paste0(summ_dir,"/","NAREAS","/","modelsMets.csv"),row.names=F,quote = F)

############################################

for(i in 1:nrow(VALID_MODELS)){
cat("copying ",paste0(mx_out,"/","models","/",as.character(VALID_MODELS[i,1]),"/","projections","/","NAREA_APPROACH","/",VALID_MODELS[i,"FILENAME"]),"\n")
  
file.copy(from=paste0(mx_out,"/","models","/",as.character(VALID_MODELS[i,1]),"/","projections","/","NAREA_APPROACH","/",VALID_MODELS[i,"FILENAME"]),
          to=paste0(mx_out,"/","models","/",as.character(VALID_MODELS[i,1]),"/","projections","/","NAREA_APPROACH","/",as.character(VALID_MODELS[i,1]),"_worldclim2_5_EMN_PA.asc.gz"),
          overwrite =T)
cat("NEW FILE: ",paste0(mx_out,"/","models","/",as.character(VALID_MODELS[i,1]),"/","projections","/","NAREA_APPROACH","/",VALID_MODELS[i,"FILENAME"]),"\n")
