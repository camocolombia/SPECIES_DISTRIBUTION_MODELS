...
test_ensemble_function<-function(spID,inputDir,county){
  ...
  outFolder <- paste(mxe_out,"/",spID,sep="")
  proj_dir<-paste(outFolder, "/", "projections",sep="")  
  proj_dir_2<-paste0(proj_dir,"/","NAREA_APPROACH");if (!file.exists(proj_dir_2)) {dir.create(proj_dir_2)}
  crossval_dir<- paste(outFolder, "/", "crossval",sep="")  
  
# if(file.exists(paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PR_ensemble.asc"))& file.exists(paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PA_SUM_ensemble.asc"))){
...
#}else{
   ...
   ENM_PA<-raster(paste(outFolder, "/", "projections","/",spID,"_worldclim2_5_EMN_PR_MEAN.asc",sep=""))
   ## ENM_PA<-zipRead(paste0(outFolder, "/", "projections"),paste0(spID,"_worldclim2_5_EMN_PR_MEAN.asc.gz"))
   if(file.exists(paste0(NAREA_COUNTY_DIR,"/",spID,"/","narea.asc.gz"))){
    ...
    if(county==F){
      ...
      if(file.exists(paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PR_ensemble.asc"))){
        writeRaster(result_PR, filename=paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PR_ensemble.asc"), overwrite=T)
        }else{
        writeRaster(result_PR, filename=paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PR_ensemble.asc"), overwrite=F)
        }
    ...
    if(county==F){
      ...
      if(file.exists(paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PR_ensemble.asc"))){
        writeRaster(result, filename=paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PA_SUM_ensemble.asc"), overwrite=T)
      }else{
        writeRaster(result, filename=paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PA_SUM_ensemble.asc"), overwrite=F)
      }
    ...
    if(county==F){
      cat("using county approach for ",as.character(spID),"\n")
      proj_dir_2<-paste0(proj_dir,"/","NAREA_APPROACH");if (!file.exists(proj_dir_2)) {dir.create(proj_dir_2)}
           # writeRaster(sum_met, filename=paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PA_MET_ensemble.asc"), overwrite=T)
            if(file.exists(paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PA_MET_ensemble.asc"))){
              writeRaster(sum_met, filename=paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PA_MET_ensemble.asc"), overwrite=T)
            }else{
              writeRaster(sum_met, filename=paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PA_MET_ensemble.asc"), overwrite=F)
            }
...
mclapply2(1:length(spList),fun=function(i){
  cat("running...",as.character(spList[[i]]),"\n")
...
lapply(1:length(spList),function(i){
  cat("running...",as.character(spList[[i]]),"\n")
  x<-test_ensemble_function(spID=spList[[i]],inputDir=inputDir,county=F)
...
#MEDIAN_ENSEMBLE_APPROACH<-function(inputDir,spList,county){
  ...
  predictions <- lapply(1:length(spList), function(i){
   ...
   # inputDir<-crop_dir
    ...
    cat("loading MEDIAN raster for ",spID," | ",i,"\n")
    model <- raster(paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PR_ensemble.asc"))
    gc();
    ...
    occVal$observed <- 1  
    #
    # proj_dir_2<-paste0(proj_dir,"/","NAREA_APPROACH")
    # ENM_PA<-raster(paste(outFolder, "/", "projections","/",spID,"_worldclim2_5_EMN_PR_MEAN.asc",sep=""))
    if(file.exists(paste0(NAREA_COUNTY_DIR,"/",spID,"/","narea.asc.gz"))){
      N_AR<-zipRead(path=paste0(NAREA_COUNTY_DIR,"/",spID),fname="narea.asc.gz")
    }else{
    ...
    bckVal <- as.data.frame(cbind(bckVal,raster::extract(N_AR,bckVal)))
    ...
    allVal <- as.data.frame(cbind(allVal, raster::extract(model,allVal[,c("lon","lat")])))
  ...
  for(i in 1:length(spList)){
    ...
    spIDM<-spList[[i]]
   # spIDM<-spList[[23]]
    cat("PA MEDIAN METRICS FOR",spIDM,"\n")
  ...
  mx_out <- paste(inputDir,"/maxent_modeling",sep="")
  summ_dir<-paste0(mx_out,"/","summary-files"); if (!file.exists(summ_dir)) {dir.create(summ_dir)}
  summ_dir_2<-paste0(summ_dir,"/","NAREAS");if (!file.exists(summ_dir_2)) {dir.create(summ_dir_2)}
  write.csv(results, paste0(summ_dir_2,"/","ensemble_metrics_PA_MEDIAN.csv"),row.names=F,quote = F)
 ...
 lapply(1:length(spList),function(i){
  #i=1
    ...
    PR_MODEL <- raster(paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PR_ensemble.asc"))
    ...
    if(file.exists(paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PA_MET_ensemble.asc"))){
      writeRaster(PR_MODEL, filename=paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PR_THR_ensemble.asc"), overwrite=T)
    }else{
      writeRaster(PR_MODEL, filename=paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PR_THR_ensemble.asc"), overwrite=F)
    }
   #writeRaster(PR_MODEL, filename=paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PR_THR_ensemble.asc"), overwrite=T)
...
#PA_SUM_ENSEMBLE_APPROACH<-function(inputDir,spList,county){
  ...
  predictions_PA_SUM <- lapply(1:length(spList), function(i){
    ...
    proj_dir<-paste(outFolder, "/", "projections",sep="")  
    proj_dir_2<-paste0(proj_dir,"/","NAREA_APPROACH")
    cat("loading PA SUM raster for ",spID,"\n")
    model <- raster(paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PA_MET_ensemble.asc"))
    ...
    #
    proj_dir_2<-paste0(proj_dir,"/","NAREA_APPROACH")
    #ENM_PA<-raster(paste(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PR_MEAN.asc",sep=""))
   ## ENM_PA<-raster(paste(outFolder, "/", "projections","/",spID,"_worldclim2_5_EMN_PR_MEAN.asc",sep=""))
    #N_AR<-zipRead(path=paste0(NAREA_COUNTY_DIR,"/",spID),fname="narea.asc")
  ...
  write.csv(results2, paste0(summ_dir_2,"/","ensemble_metrics_PA_SUM.csv"),row.names=F,quote = F)
...
#MEAN_ENSEMBLE_APPROACH<-function(inputDir,spList,county){
  ...
  predictions_PA_MEAN <- lapply(1:length(spList), function(i){
    ...
    outFolder <- paste(mxe_out,"/",spID,sep="")
    proj_dir<-paste(outFolder, "/", "projections",sep="") 
    cat("loading PA MEAN raster for ",spID,"\n")
  ...
  for(i in 1:length(spList)){
    ...
    if(unique(z$observed)>1){
    spIDM<-spList[[i]]
    cat("PA MEAN METRICS FOR",spIDM,"\n")
    z <- z[,c("spID","observed","predicted")]
    ...
    predictions5[i,11]<-PresenceAbsence::predicted.prevalence(z,threshold =PresenceAbsence::optimal.thresholds(z,opt.methods = 9)[2])[3]
      ...
      z <- z[,c("spID","observed","predicted")]
      predictions5[i,1]<-spIDM
      predictions5[i,2]<-0.5
      predictions5[i,3]<-1
      predictions5[i,4]<-0.5
      predictions5[i,5]<-0
      predictions5[i,6]<-0
      predictions5[i,7]<-0
      predictions5[i,8]<-0
      predictions5[i,9]<-(PresenceAbsence::sensitivity(cmx(z))[1]+PresenceAbsence::specificity(cmx(z))[1])-1
      predictions5[i,10]<-0.5
      predictions5[i,11]<-0
  ...
  write.csv(results3, paste0(summ_dir_2,"/","ensemble_metrics_PA_MEAN.csv"),row.names=F,quote = F)
  ...
  lapply(1:length(spList),function(i){
    ...
    if(county==T){
      ...
      PR_MODEL<-PR_MODEL*N_AR
      
    }
    proj_dir_2<-paste0(proj_dir,"/","NAREA_APPROACH");if (!file.exists(proj_dir_2)) {dir.create(proj_dir_2)}
    
   # writeRaster(PR_MODEL, filename=paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PA_mean.asc"), overwrite=T)
    if(file.exists(paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PA_mean.asc"))){
      writeRaster(PR_MODEL, filename=paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PA_mean.asc"), overwrite=T)
    }else{
      writeRaster(PR_MODEL, filename=paste0(proj_dir_2,"/",spID,"_worldclim2_5_EMN_PA_mean.asc"), overwrite=F)
    }
...
#}
  ...
  lapply(1:length(spList),function(i){
    spID=spList[[i]]
    cat("compressing files of ",spID,"\n")
    x<-compress_function(spID=spList[[i]],inputDir=inputDir,OSys=as.character(get_os()))
    
  });gc()
  
  
  
  compress_function_NAREA<-function(spID,inputDir,OSys){
    # inputDir<-crop_dir
    mxe_out <- paste(inputDir,"/maxent_modeling/models",sep="")
    outFolder <- paste(mxe_out,"/",spID,"/projections/NAREA_APPROACH",sep="")
    
    
    ftoZIP <- list.files(paste(outFolder, sep=""), pattern=".asc$")
    cat("Compressing... \n")
    for (fz in ftoZIP) {
  ...
  lapply(1:length(spList),function(i){
    spID=spList[[i]]
    cat("compressing files of ",spID,"\n")
    x<-compress_function_NAREA(spID=spList[[i]],inputDir=inputDir,OSys=as.character(get_os()))
